...
function getPromptChoices(game: IGame, joker?: boolean) {
  const choices = [];
  ...
  if (joker) {
    [
      ...
      if ([
        ...
        choices.push({
          ...
          name: category,
          value: category,
          hint: calculateScore(category, game.diceRoll),
        });
      }
    });
  } else {
    Object.keys(game.score).forEach(key => {
      ...
      if (category === YahtzeeScoreCategory.BonusYahtzees) {
        const disabled = game.score[YahtzeeScoreCategory.Yahtzee] === null
          || calculateScore(category, game.diceRoll) === 0;
        choices.push({
          message: scoreLabels[category],
          name: category,
          value: category,
          hint: `[${game.score[category]}]${(!disabled &&
...
export default async function handleScoreDiceMode(game: IGame, joker?: boolean): Promise<IGame> {
  const _game = { ...game };
  ...
  return prompt.run().then((answer) => {
    ...
    if (category === YahtzeeScoreCategory.BonusYahtzees
      ...
      return handleScoreDiceMode(_game, joker);
    } else {
